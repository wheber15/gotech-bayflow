...
export function UserManagement({ users, reports, trailers, currentUserId, onSetActive, onBlock, onUnlock }: Props) {
  ...
  return <section className="userPanel"><div><h2>Users</h2><p>Manage prototype account access and review recorded activity.</p></div><div className="userList">{users.map(user => { const locked = isUserLocked(user); return <article className="userCard" key={user.id}><button className="userOpenBtn" onClick={() => setSelectedId(user.id)}><UserRound size={21}/><span><b>{user.displayName}</b><small>@{user.username}</small></span></button><dl><div><dt>Role</dt><dd>{user.role.replace("_", " ")}</dd></div><div><dt>Account</dt><dd className={user.active ? "activeText" : "lockedText"}>{user.active ? "Active" : "Inactive"}</dd></div><div><dt>Access</dt><dd className={locked ? "lockedText" : "activeText"}>{locked ? <><LockKeyhole size={15}/>Locked</> : "Available"}</dd></div><div><dt>Failed</dt><dd>{user.failedLoginAttempts}</dd></div></dl><button className="detailsBtn" onClick={() => setSelectedId(user.id)}>View user</button></article> })}</div>{selected && <UserDetails user={selected} currentUserId={currentUserId} reports={reports} trailers={trailers} onClose={() => setSelectedId(undefined)} onSetActive={onSetActive} onBlock={onBlock} onUnlock={onUnlock}/>}</section>;
}
...
function UserDetails({ user, currentUserId, reports, trailers, onClose, onSetActive, onBlock, onUnlock }: { user: AppUser; currentUserId: string; reports: FloorReport[]; trailers: Trailer[]; onClose: () => void; onSetActive: (id: string, active: boolean) => void; onBlock: (id: string) => void; onUnlock: (id: string) => void }) {
  ...
  return <div className="detailsBackdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) onClose() }}><section className="reportDetails userDetails" role="dialog" aria-modal="true" aria-labelledby="user-details-title"><div className="formTitle"><div><span className="eyebrow">USER ACCOUNT</span><h2 id="user-details-title">{user.displayName}</h2><p>@{user.username}</p></div><button className="closeBtn" onClick={onClose} aria-label="Close user details"><X size={22}/></button></div><dl className="reportMeta"><div><dt>Role</dt><dd>{user.role.replace("_", " ")}</dd></div><div><dt>Account</dt><dd>{user.active ? "Active" : "Inactive"}</dd></div><div><dt>Lock status</dt><dd>{locked ? "Locked" : "Available"}</dd></div><div><dt>Failed attempts</dt><dd>{user.failedLoginAttempts}</dd></div><div><dt>Last login</dt><dd>{formatDate(user.lastLoginAt)}</dd></div><div><dt>Last unlock</dt><dd>{user.unlockedAt ? `${formatDate(user.unlockedAt)} by ${user.unlockedBy}` : "—"}</dd></div></dl><div className="accountActions"><button className="secondaryBtn" disabled={user.id === currentUserId} onClick={() => onSetActive(user.id, !user.active)}>{user.active ? <><Ban size={17}/>Deactivate</> : <><CheckCircle2 size={17}/>Activate</>}</button>{locked ? <button className="unlockBtn" onClick={() => onUnlock(user.id)}><UnlockKeyhole size={17}/>Unlock</button> : <button className="blockBtn" disabled={user.id === currentUserId} onClick={() => onBlock(user.id)}><LockKeyhole size={17}/>Block</button>}</div><div className="activitySection"><h3><Activity size={18}/>Recent activity</h3>{activities.length ? <ul>{activities.map((activity, index) => <li key={`${activity.label}-${index}`}><b>{activity.label}</b><span>{formatActivityTime(activity.time)}</span></li>)}</ul> : <p>No recorded activity in this prototype session.</p>}</div></section></div>;
}
